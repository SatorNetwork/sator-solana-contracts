...
pub fn initialize_stake(        
    ...
    let token_account = Pubkey::create_with_seed(&stake_authority.0, "ViewerStake::token_account", &program_id())?;
...
pub fn lock(        
    ...
    let token_account_stake_target = Pubkey::create_with_seed(&stake_authority, "ViewerStake::token_account", &program_id())?;
    let lock_account = Pubkey::create_with_seed_for_pubkey(&stake_authority, wallet, &program_id())?;
    Ok(solana_program::instruction::Instruction::new_with_borsh(
        crate::id(),
...
pub fn unlock(        
    ...
    let token_account_stake_source = Pubkey::create_with_seed(&stake_authority, "ViewerStake::token_account", &program_id())?;
    let lock_account = Pubkey::create_with_seed_for_pubkey(&stake_authority, wallet, &program_id())?;
    Ok(solana_program::instruction::Instruction::new_with_borsh(
        crate::id(),
