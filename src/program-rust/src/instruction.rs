...
pub fn lock(        
    ...
    let stake_authority = Pubkey::find_program_address_for_pubkey(stake_owner, &program_id());
    let token_account_stake_target = Pubkey::create_with_seed(&stake_authority.0, "ViewerStake::token_account", &program_id())?;
    let lock_account = Pubkey::create_with_seed_for_pubkey(&stake_authority.0, wallet, &program_id())?;
    Ok(solana_program::instruction::Instruction::new_with_borsh(
        crate::id(),
